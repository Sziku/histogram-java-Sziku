...
public class Histogram {
    ...
    public void normalizeValues() {
        ...
        for (Map.Entry<Range, Integer> entry : histogram.entrySet()){
            System.out.println("histogram erteke: "+entry.getValue());
            normalValue = (entry.getValue()) * 100 / (max);
            System.out.println("histogram normal erteke: "+ normalValue);
            System.out.println("-----------");
            histogram.put(entry.getKey(), normalValue);
        }
    }
    public int min() {
        Map.Entry<Range, Integer> iniEntry = histogram.entrySet().iterator().next();
        int min = iniEntry.getValue();
